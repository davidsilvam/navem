...
import libnavem.img_utils as img_utils
#fim_teste

#ini class and functions
class NavemDataGenerator(ImageDataGenerator):
    """
...
class NavemDirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        #debug dataset
        #for i in zip(self.filenames, self.ground_truth):
        #    print(i)
        ...
        super(NavemDirectoryIterator, self).__init__(self.samples,
    ...
    def _decode_experiment_dir(self, dir_subpath):
        ...
        gyro_filename = os.path.join(dir_subpath, "gyro.txt")
        ...
        try:
            ground_truth = np.loadtxt(gyro_filename, usecols=[1],
            ...
            try:
                ...
                print("Neither gyros nor labels found in dir {}".format(
                dir_subpath))
                raise IOError
    ...
    def _get_batches_of_transformed_samples(self, index_array) :
        ...
        batch_gyro = np.zeros((current_batch_size,), dtype=K.floatx())
        ...
        for i, j in enumerate(index_array):
            ...
            if self.exp_type[index_array[i]] == 1:
                ...
                #batch_gyro[i,0] = 1.0#Clocar para gyro
                batch_gyro[i] = self.ground_truth[index_array[i]]
                #batch_coll[i] = np.array([1.0, 0.0])
            else:
                # Collision experiment (t=0)
                #batch_gyro[i] = np.array([0.0, 0.0])
                a = 1
                #batch_coll[i,0] = 0.0
...
def write_divided_dataset(exp_model_path, train, test):
    trainFile = open(exp_model_path + "/train.txt", "w")
    testFile = open(exp_model_path + "/val.txt", "w")
    for sample in train.values:
        trainFile.write(sample[0] + " " + str(sample[1]) + "\n")
    for sample in test.values:
        testFile.write(sample[0] + " " + str(sample[1]) + "\n")
    trainFile.close()
    testFile.close()
